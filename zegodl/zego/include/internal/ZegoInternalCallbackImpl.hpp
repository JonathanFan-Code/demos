...
#define oInternalCallbackCenter ZegoSingleton<ZegoInternalCallbackCenter>::CreateInstance()
        ...
        public:
            ...
            declearMultiRawMember(ZegoRoomSetRoomExtraInfoCallback);
            declearMultiRawMember(ZegoPublisherSetStreamExtraInfoCallback);
            declearMultiRawMember(ZegoPublisherUpdateCdnUrlCallback);
            declearMultiRawMember(ZegoIMSendBroadcastMessageCallback);
            declearMultiRawMember(ZegoIMSendBarrageMessageCallback);
            declearMultiRawMember(ZegoIMSendCustomCommandCallback);
            ...
            void registerCallback()
            {
                ...
                oInternalOriginBridge->registerRoomStreamUpdateCallback(ZegoVoidPtr(&ZegoInternalCallbackCenter::zego_on_stream_update), ZegoVoidPtr(this));
            ...
            static void zego_on_engine_state_update(zego_engine_state state, void *user_context)
            {
                ...
                auto handler = weakHandler.lock();
                if (handler)
                    handler->onEngineStateUpdate(ZegoEngineState(state));
                ZEGO_SWITCH_THREAD_ING
            }
            ...
            static void zego_on_room_online_user_count_update(const char *room_id, int online_user_count, void *user_context)
            {
                ...
                auto handler = weakHandler.lock();
                if (handler)
                    handler->onRoomOnlineUserCountUpdate(roomID, online_user_count);
                ZEGO_SWITCH_THREAD_ING
            }
            ...
            static void zego_on_room_extra_info_update(const char *room_id, const struct zego_room_extra_info *room_extra_info_list, unsigned int room_extra_info_count, void *user_context)
            {
                ...
                auto handler = weakHandler.lock();
                if (handler)
                    handler->onRoomExtraInfoUpdate(roomID, infoList);
                ZEGO_SWITCH_THREAD_ING
            }
            ...
            static void zego_on_player_recv_sei(const char *stream_id, const unsigned char *data, unsigned int data_length, void *user_context)
            {
                ...
                auto handler = weakHandler.lock();
                if (handler)
                    handler->onPlayerRecvSEI(streamID, seiData.data(), ZegoUInt(seiData.size()));
                ZEGO_SWITCH_THREAD_ING
            }
            //#endif
            ...
            static void zego_on_remote_audio_data(const unsigned char *data, unsigned int data_length, zego_audio_frame_param _param, void *user_context)
            {
                ZEGO_UNUSED_VARIABLE(user_context);
                auto handler = oInternalCallbackCenter->getIZegoAudioDataHandler();
                if (handler)
