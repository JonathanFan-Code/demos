        ...
        public:
            ...
            void zego_on_audio_effect_play_state_update(unsigned int audio_effect_id, enum zego_audio_effect_play_state state, zego_error error_code)
            {
                ...
                if (aepEventHandler)
                {
                    ...
                    auto aepEventHandler = weakAEPEventHandler.lock();
                    if (aepEventHandler)
                    {
                        aepEventHandler->onAudioEffectPlayStateUpdate(this, audio_effect_id, ZegoAudioEffectPlayState(state), error_code);
                    }
