...
#define declearMultiRawMember(MemberType)                                 \
    std::unordered_map<zego_seq, MemberType> m##MemberType;               \
    std::recursive_mutex m##MemberType##Mutex;                            \
                                                                          \
    void insert##MemberType(zego_seq seq, MemberType member)              \
    {                                                                     \
        std::lock_guard<std::recursive_mutex> lock(m##MemberType##Mutex); \
        m##MemberType.insert({seq, member});                              \
    }                                                                     \
                                                                          \
    MemberType get##MemberType(zego_seq seq)                              \
    {                                                                     \
        std::lock_guard<std::recursive_mutex> lock(m##MemberType##Mutex); \
        ...
        return member;                                                    \
                                                                          ...
                                                                          \
    MemberType erase##MemberType(zego_seq seq)                            \
    {                                                                     \
        std::lock_guard<std::recursive_mutex> lock(m##MemberType##Mutex); \
